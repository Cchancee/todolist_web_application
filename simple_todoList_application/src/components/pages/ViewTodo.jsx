...
const ViewTodo = ({ todos, Setodos }) => {
    ...
    return <>
        <div className="columns-9 items-center cursor-pointer  top-0 text-center bg-orange-800 text-white p-3 h-14 rounded-t-md">
            ...
            {reversedTodos.map((todo, index) => (
                <div key={index} className="columns-3 text-center  bg-slate-200 cursor-pointer text-black p-2">
                    ...
                    <div className="w-full">
                        {!todo.todoStatus ? <p className="text-center rounded-full w-fit pl-2 pr-2 p-1 text-white font-normal text-xs bg-orange-700 "><i className="fa fa-question-circle"></i> Not Done</p> : <p className="text-center rounded-full w-fit pl-2 pr-2 p-1 text-white font-normal text-xs bg-green-500 "><i className="fa fa-check-circle"></i> Done</p>}
                    </div>
